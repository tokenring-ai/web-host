...
import {z} from "zod";
import {type BunRequest, type BunResponse, type BunRouter, type WebResource} from "./types.ts";
...
export default class JsonRpcResource implements WebResource {
  ...
  async register(router: BunRouter): Promise<void> {
    router.post(this.jsonRpcEndpoint.path, async (request: BunRequest, response: BunResponse) => {
      try {
        const body = await request.json();
        const {jsonrpc, id, method, params = []} = jsonBodySchema.parse(body);
        ...
        try {
          ...
          return response.json({jsonrpc: "2.0", id, result: validatedResult});
        } catch (error: any) {
          return response.json({jsonrpc: "2.0", id, error: {code: -32603, message: error.message}});
        }
      } catch (error: any) {
        return response.json({jsonrpc: "2.0", id: null, error: {code: -32700, message: error.message}});
      }
    });
  }
